...
func CreateConnectionString(dataType string, host string, port int, database string, username, password string) string {
	...
	case "clickhouse":
		...
		return fmt.Sprintf("DataType=clickhouse,PoolMaxSize=50,PoolMinSize=1,ConnectionString=clickhouse://%s:%s@%s:%d/%s?dial_timeout=10s&read_timeout=20s", username, password, host, port, database)
	}
	return ""
}
